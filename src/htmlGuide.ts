...
export async function generateBuildGuide(
  ...
  const treeObj = (treeResp.tree ?? {}) as { nodes?: number[]; treeVersion?: string };
  ...
  // Raw tree for icon paths (TreeNode doesn't carry `icon`)
  const rawTree = JSON.parse(
    readFileSync(path.join(forkPath, "TreeData", treeVersion, "tree.json"), "utf8")
  ) as { nodes: Record<string, { icon?: string }> };
