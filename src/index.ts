  ...
  // ----- Phase 8G: build synthesis -----
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        secondarySkills: { type: "array", items: { type: "string" }, description: "Secondary DAMAGE skills (e.g. ['Tempest Bell']). Each gets its own socket group + engine-screened supports and is flagged into Full DPS, so the build is evaluated as a synergistic whole." },
    ...
    switch (name) {
      ...
      case "synthesize_build": {
        ...
        const result = await synthesizeBuild(b, fp, {
          ...
          secondarySkills: Array.isArray(a.secondarySkills) ? a.secondarySkills.filter((s): s is string => typeof s === "string") : undefined,
