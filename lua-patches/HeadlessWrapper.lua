...
if POB_SCRIPT_DIR ~= '' then
  ...
  -- Add runtime/lua so xml, dkjson, etc. resolve without external LUA_PATH
...
-- Start the stdio server if requested, then exit (don't fall through to GUI bootstrap)
if os.getenv('POB_API_STDIO') == '1' or has_flag('--stdio') then
  -- Standard bootstrap (same as the normal path below)
  dofile("Launch.lua")
  ...
  -- Expose helpers as globals so API handlers can use them
  _G.build = mainObject.main.modes["BUILD"]
  function _G.newBuild()
    mainObject.main:SetMode("BUILD", false, "API")
    runCallback("OnFrame")
  end
